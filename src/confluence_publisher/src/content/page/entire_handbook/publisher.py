...
from content.page.entire_handbook.diagram.publisher import ServiceDiagramPublisher
from content.page.entire_handbook.view import EntireHandbookPageView
...
from data.confluence.model.title import ServiceHandbookManualPageTitle, ServiceHandbookPageTitle, \
    ServiceHandbookOldManualPageTitle
...
class EntireHandbookPagePublisher:
    __branch: Branch

    __html_templates_storage: HtmlTemplatesStorage

    __page_view: EntireHandbookPageView
    ...
    def __init__(self,
        ...
        self.__page_view = EntireHandbookPageView(html_templates_storage)
    ...
    async def __create_handbook_manual_page(self, spec: ServiceSpecExt, parent_page: ConfluencePage):
        if spec.settings.confluence.clear_manual_page_title is not None:
            old_page = await self.__confluence.pages.find(ServiceHandbookOldManualPageTitle.title(spec),
                                                  spec.settings.confluence.space)
            if old_page is not None:
                await self.__confluence.pages.delete(old_page.id)

        page = await self.__confluence.pages.find(ServiceHandbookManualPageTitle.title(spec),
                                                  spec.settings.confluence.space)
        if page is not None:
            return
        page = ConfluencePage()
    ...
    async def publish(self,
        ...
        diagramPublisher = ServiceDiagramPublisher(self.__confluence,
                                                   spec.settings)
        await diagramPublisher.publish(page, spec)
        return page
