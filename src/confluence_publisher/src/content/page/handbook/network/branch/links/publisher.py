...
from content.page.handbook.network.branch.links.view import NetworkBranchLinksView
...
class NetworkBranchLinksPublisher(NetworkPagePublisherCore):
    __branch: Branch

    __html_templates_storage: HtmlTemplatesStorage

    __page_view: NetworkBranchLinksView
    ...
    def __init__(self,
        ...
        self.__page_view = NetworkBranchLinksView(html_templates_storage)

    @staticmethod
    def title(spec: ServiceSpecExt, branch: Branch):
        return NetworkBranchLinksPageTitle.title(spec, branch)
