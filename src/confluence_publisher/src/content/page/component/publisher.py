...
from data.confluence.model.title import SystemNetworkBranchDiagramPageTitle, ComponentBranchDiagramPageTitle
from data.confluence.service import ConfluenceService
from data.template.templates_storage import HtmlTemplatesStorage


class ComponentBranchDiagramPublisher:
    __branch: Branch
    ...
    def title(branch: Branch):
        return ComponentBranchDiagramPageTitle.title(branch)
    ...
    async def publish(self, parent_page: ConfluencePage) -> ConfluencePage:
        ...
        diagram_file_name = self.__specs_settings.component_diagram_fname
